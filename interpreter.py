"""
Autor: Scott.J.M.R Timmermans

Scolang Interpreter

Supports:
inline comments everything between # <?.?> # does not work on multiple lines

if, while loops, print

conditional operators        [ < > <= >= == != ]
arithmitic operators         [ = + - % * ** / ]

all operators support        [int,float] 
operators that support [str] [ + = < > <= >= == != ]

See "token_types.py" for syntax
...
def visit(node : node,program_state : program_state) -> Union[program_state,Union[int,float]]:
    ...
    This function returns a number(float or int) if a number_node or name_node is visited otherwise a program_state.
    """
    ...
    if isinstance(node, (if_node,while_node)):
...
def main():
    ...
    # run program
    print("Length of program: {0} instructions".format(len(program)))
...
class run:
    ...
    def run_program(self,program : List[node],program_state : program_state) -> program_state:
        ...
        """
        # Uncomment for every state of the program
