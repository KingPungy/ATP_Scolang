...
def parse_tokens_to_nodes(tokens : List[token]) -> List[node]:
    ...
    This is also true for the if and while nodes
    The integer and name nodes will be filed with the corresponding value
    """
    if len(tokens) == 0:
        return []
    a, *tail = tokens
    ...
    if a.token_type in (token_types.IF_STATEMENT,):
        return [if_node(node,a.value,0)] + parse_tokens_to_nodes(tail) 
    elif a.token_type == token_types.NUMBER:
        return [number_node(a)] + parse_tokens_to_nodes(tail) 
    elif a.token_type == token_types.END_IF:
        return [end_if_node()] + parse_tokens_to_nodes(tail)
    elif a.token_type == token_types.WHILE_START:
        return [while_node(node,a.value,0)] + parse_tokens_to_nodes(tail)
    elif a.token_type == token_types.WHILE_END:
        return [end_while(0)] + parse_tokens_to_nodes(tail)
...
def check_single_sided(a : node, b : node, node_type : node) -> List[node]:
    """
    Check if a single sided operator(op_if,op_print) is located at a if so fill the operator with the node located at b
...
def parse_row_tokens(tokens : List[token]) -> List[node]:
    ...
    - parse single sided (if,while,print)
    """
    ...
    output = parse_single_sided(parse_single_sided(parse_single_sided(operator_output,if_node),while_node),print_node)
    return output[0]
...
def fill_condition(nodes : List[node], node_type : node, end_node_type : node) -> List[node]:
    """
    function to fill the conditional nodes(while,if) with the amount of rows they need to skip.
    ...
    the node_type is the start node(if_node,while_node)
    the end_node_type is the end node(end_if_node,end_while_node)
    """
    if len(nodes) == 1:
        return nodes
    a, *tail = nodes
...
def fill_end_condition(nodes : List[node], node_type : node, end_node_type : node) -> List[node]:
    """
    Reverse of above function to fill the row number of the end_while_node because it needs to go back to the while to implement loops
    """
    if len(nodes) <=1:
        return nodes
    *head, a = nodes
...
def parse_program(lex_output : List[token]) -> List[node]:
    ...
    final_output = fill_end_condition(fill_condition(fill_condition(op_output,if_node,end_if_node),while_node,end_while),end_while,while_node)
    return final_output
